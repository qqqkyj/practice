...
const getProductReviewsById = async (productId) => {
	const config = { url: `${BASE_URL}/products/${productId}`, method: "get" };
...
const getProductReviewRatingById = async (productId) => {
	const config = { url: `${BASE_URL}/products/${productId}`, method: "get" };
	const res = await axios(config);
	const reviews = res["data"]["reviews"];
...
// 2. 응답 데이터를 출력한다
const getAllProductsTagList = async () => {
	const config = { url: `${BASE_URL}/posts`, method: "get" };
	const res = await axios(config);
	const posts = res["data"]["posts"];
	let tagList = [];

	// ... : 스프레드 연산자를 사용하여 tags 배열 모두 합치기
	posts.forEach((post) => {
...
// 2. 응답 데이터를 출력한다
const getPostsByTag = async (searchTag) => {
	const config = { url: `${BASE_URL}/posts/tag/${searchTag}`, method: "get" };
	const res = await axios(config);
...
const searchProducts = async (searchTitle) => {
	const config = {
		url: `${BASE_URL}/products/search?q=${searchTitle}`,
		method: "get",
	};
	const res = await axios(config);
