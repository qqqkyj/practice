...
const getRecipesByMeal = async (searchTitle) => {
	const res = await axios.get(`${BASE_URL}/recipes/meal-type/${searchTitle}`);
	console.log(res["data"]);
};

//getRecipesByMeal("lunch");
...
const addRecipe = async (config) => {
	const res = await axios(config); //post로 레시피 추가
	console.log(res["data"]);
};
...
const searchUsers = async (userName) => {
	const res = await axios.get(`${BASE_URL}/users/search?q=${userName}`);
...
const getAllTodosByUserId = async (userId) => {
	const res = await axios.get(`${BASE_URL}/todos/user/${userId}`);
	console.log(res["data"]["todos"]);
};

//getAllTodosByUserId(9);
