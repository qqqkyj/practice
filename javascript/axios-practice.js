...
const getASingleCart = async (cartNum) => {
	const res = await axios.get(`${BASE_URL}/${cartNum}`);
	const cart = res["data"];
	//console.table(Object.entries(data));
	console.log(cart);
};
...
const getASingleCartTotalProductsCnt = async (cartNum) => {
	const res = await axios.get(`${BASE_URL}/${cartNum}`);
	const data = res["data"];
	//console.table(Object.entries(data));
	console.log(data["totalProducts"]);
};
getASingleCartTotalProductsCnt(10);
...
const getASingleCartProducts = async (cartNum) => {
	const res = await axios.get(`${BASE_URL}/${cartNum}`);
	const data = res["data"];
	//console.table(Object.entries(data));
	console.log(data["products"]);
};
getASingleCartProducts(10);
...
const getASingleCartProductNames = async (cartNum) => {
	const res = await axios.get(`${BASE_URL}/${cartNum}`);
	const data = res["data"];
